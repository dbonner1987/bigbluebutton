# -----
# Virt_Collection = new Meteor.Collection("virtual_coll")
...
Template.footer.helpers
  getFooterString: ->
    ...
    # foot = "(c) #{info.copyrightYear} CSWM. [build Custom.0.9.1_CSWM  - #{info.dateOfBuild}] - For more information visit #{info.link}"
    foot = "(c) #{info.copyrightYear} CSWM. [BBB build Custom.0.9.1_CSWM]"
...
Template.header.events
  # "click .audioFeedIcon": (event) ->
    # prevents the notification from displaying until the previous one is hidden
    # if getInSession('webrtc_notification_is_displayed') is false
    #   # verifies if the browser supports WebRTC
  ...
  # "click .navbarTitle": (event) ->
  #     console.log(Meteor.Meetings.findOne().voiceConf)


  # New mp3 media embed version using shoutcast for all browser streaming
  "click .audioFeedIcon": (event) ->
      # http://url:8000/conference-77624-live.m
      confId = Meteor.Meetings.findOne()?.voiceConf or null

      if confId

        if Meteor.Users.findOne({userId: getInSession 'userId'})?.wants_audio
          ...
          Meteor.call('userUpdateWantsAudioStatus', getInSession("userId"), 1)
          console.log('User wants audio')
          if Meteor.Meetings.findOne()?.voiceRecordingActive is 1
            appendMediaPlayerForAudio(confId)
            Meteor.call('userUpdateWantsAudioStatus', getInSession("userId"), 1)
          else
            console.log('Audio recording not available')
            Meteor.call('userNeedsAudioReloaded', getInSession("meetingId"), 1)

          $('.joinAudioButton').attr('title', 'Leave Audio Call').attr('data-original-title', 'Leave Audio Call')

          # -----
          # Session.set("reloadAudio", 0)

          # -----
          # The autorun function runs but does not alert (counter: 0)
          # Tracker.autorun (computation) ->
          #   if Session.get("reloadAudio") is 1
          #     alert("reloadAudio started")
...
Template.main.helpers
	setTitle: ->
		document.title = "CSWM Webinar #{window.getMeetingName() ? ''}"
