  ...
  Meteor.call('jasmine/environmentInfo', function (error, mirrorInfo) {                                                // 4
    ...
    if (mirrorInfo.isTestPackagesMode) {                                                                               // 10
      ...
      Tracker.autorun(function () {                                                                                    // 12
        var clientAggregateReport = Velocity.Collections.AggregateReports                                              // 13
          .findOne({name: testFramework.name});                                                                        // 14
                                                                                                                       // 15
        if (clientAggregateReport) {                                                                                   // 16
          if (clientAggregateReport.result === 'completed') {                                                          // 17
            hasCompletedOnce = true;                                                                                   // 18
          } else if (hasCompletedOnce && clientAggregateReport.result === 'pending') {                                 // 19
            Reload._reload();                                                                                          // 20
          }                                                                                                            // 21
        }                                                                                                              // 22
      });                                                                                                              // 23
      ...
      Tracker.autorun(function () {                                                                                    // 25
        var serverAggregateReport = Velocity.Collections.AggregateReports                                              // 26
          .findOne({name: 'jasmine-server-integration'});                                                              // 27
                                                                                                                       // 28
                                                                                                                       // 29
        if (serverAggregateReport && serverAggregateReport.result === 'completed') {                                   // 30
          testFramework.runTests();                                                                                    // 31
        }                                                                                                              // 32
      });                                                                                                              // 33
    } else {                                                                                                           // 34
      testFramework.runTests()                                                                                         // 35
    }                                                                                                                  // 36
  });                                                                                                                  // 37
})                                                                                                                     // 38
                                                                                                                       // 39
